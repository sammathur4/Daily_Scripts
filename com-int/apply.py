...
def greenhouse(driver):
    ...
    # add linkedin
    try:
        driver.find_element_by_xpath("//label[contains(.,'LinkedIn')]").send_keys(
            JOB_APP["linkedin"]
        )
    except NoSuchElementException:
        try:
    ...
    # add website
    try:
        driver.find_element_by_xpath("//label[contains(.,'Website')]").send_keys(
            JOB_APP["website"]
        )
    except NoSuchElementException:
        pass
